...
function _byString(interval, hollaback) {
    ...
    var exit = false
    while (this.contains(current) && !exit) {
        var value = hollaback.call(this, current.clone());
        if(value === false) {
            exit = true;
        }
...
function _byRange(interval, hollaback) {
    ...
    var exit = false;
    for (var i = 0; i <= l && !exit; i++) {
        var value = hollaback.call(this, moment(this.start.valueOf() + interval.valueOf() * i));
        if(value === false) {
            exit = true;
        }
    }
